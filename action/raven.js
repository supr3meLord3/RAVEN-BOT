...
let _0x2f8982 = participants
  .filter(p => !p.admin && p.id && typeof p.id === 'string')
  .map(p => p.id)
  .filter(id => !id.startsWith(mycode) && id !== client.decodeJid(client.user.id));
...
if (!args || !args[0]) {
  ...
  for (let user of _0x2f8982) {
    _0x2d7d67 += `𓅂 @user.split("@")[0]`;
  
  _0x2d7d67 += ` 𝗿𝗲𝗺𝗼𝘃𝗲 𝘁𝗵𝗲𝗺 𝘀𝗲𝗻𝗱 foreigners -x`;
  client.sendMessage(m.chat,  text: _0x2d7d67, mentions: _0x2f8982 ,  quoted: m );
 else if (args[0] === "-x") 
  setTimeout(() => 
    client.sendMessage(m.chat, 
